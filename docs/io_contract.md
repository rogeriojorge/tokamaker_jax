...
| CLI case manifest | `tokamaker-jax cases --json` | Lists runnable examples and planned upstream parity fixtures. |
| CLI upstream fixtures | `tokamaker-jax upstream-fixtures --json` | Summarizes exact upstream TokaMaker mesh/geometry fixture files when available. |
| CLI fixed-boundary evidence | `tokamaker-jax fixed-boundary-evidence --json` | Extracts source-level evidence from upstream fixed-boundary notebooks and gEQDSK. |
...
The committed docs copy is generated by `examples/generate_assets.py` at
`docs/_static/case_manifest.json`. Every entry records:

- case id, title, status, and category;
- local path and command when runnable;
- validation gate command when applicable;
- parity level and upstream source mapping;
- committed output artifacts;
- citations and notes that limit the current claim.

### Upstream Fixture Summary
...
The committed docs copy is generated by `examples/generate_assets.py` at
`docs/_static/upstream_fixture_summary.json`. It records source-audit fixture
metadata only: mesh counts, geometry counts, bounds, region-cell counts,
region areas, SHA-256 hashes, and example-file availability. It must not be
used as a full equilibrium parity claim.

### Fixed-Boundary Upstream Evidence
...
The committed docs copy is generated by `examples/generate_assets.py` at
`docs/_static/fixed_boundary_upstream_evidence.json`. It records source-level
notebook and gEQDSK metadata only: source hashes, code/markdown cell counts,
fixed/free-boundary assignments, solve/init calls, target expressions,
profile-matching indicators, gEQDSK dimensions, flux range, q range, axis,
current, and center-field values. It must not be used as a full
fixed-boundary equilibrium parity claim.
...
| `case_manifest_status.png` | `examples/generate_assets.py` | Runnable/planned case status counts. |
...
| `upstream_fixture_mesh_sizes.png` | `examples/generate_assets.py` | Upstream mesh node/cell inventory. |
| `fixed_boundary_upstream_geqdsk.png` | `examples/generate_assets.py` | Upstream fixed-boundary gEQDSK source flux map. |
| `io_artifact_map.png` | `examples/generate_assets.py` | Input/output artifact flow. |
| `case_manifest.json` | `examples/generate_assets.py` | Case inventory used by CLI, GUI, docs, and tests. |
| `upstream_fixture_summary.json` | `examples/generate_assets.py` | Exact upstream fixture mesh/geometry summaries. |
| `fixed_boundary_upstream_evidence.json` | `examples/generate_assets.py` | Upstream fixed-boundary source-evidence report. |
