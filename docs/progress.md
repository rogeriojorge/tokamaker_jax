...
declarations are intentionally unversioned at the project metadata level. The
CLI, GUI, docs, and tests now also share a typed case manifest for runnable
examples and planned upstream parity fixtures. The docs also include an
availability-gated inventory of exact upstream TokaMaker mesh and geometry
fixture files extracted from the local OpenFUSIONToolkit checkout.

## Status

| Lane | Approx. status | Current state |
| --- | ---: | --- |
| M1 mesh/geometry | 99% | `TriMesh`, OFT-style mesh I/O, region primitives, TOML region parsing, canonical sample regions, rectangular triangular refinement fixtures, geometry previews, upstream TokaMaker mesh/geometry fixture inventory, and IO/architecture docs exist; production-grade mesh generation and limiter/divertor topology policies move to the next milestone. |
...
| Docs/examples | 100% | Installation, architecture, equations, derivations, design decisions, IO/artifact contracts, case manifest, upstream fixture inventory, porting map, API, examples, progress, validation manifest, upstream/literature comparisons, benchmark threshold docs, GUI/report notes, literature reproduction docs, and README quick-start are complete for this staged milestone. |
| Config/CLI | 100% | Default install now includes GUI dependencies, dependency metadata is unversioned, TOML runs support solver/grid/source/coil/output settings plus region geometry, `tokamaker-jax validate` checks configs without solving, `tokamaker-jax verify` runs all implemented gates, `tokamaker-jax cases` lists runnable examples plus planned upstream fixtures, and `tokamaker-jax upstream-fixtures` summarizes exact source fixture files when available. |
...
| GUI | 99% | NiceGUI seed equilibrium controls, region-geometry preview, metadata summaries, region tables, validation convergence plots, reduced coil-response plots, workflow-dashboard summaries, case browser with read-only source preview, stored validation/OFT/benchmark/upstream-fixture report tables, and default-install GUI support are complete for this staged milestone; editable TOML authoring and one-click run orchestration move to the next milestone. |
...
1. Promote upstream fixture inventory into exact mesh/geometry structural
   parity tests that are available in CI without a local OFT checkout.
...
4. Add hardware-normalized benchmark history for sparse assembly, matrix-free
   apply, profile iteration, circular-loop response, and manufactured
   Grad-Shafranov solves.
5. Promote the GUI case browser into an editable TOML authoring and one-click
   runner surface while preserving the current manifest-backed reproducibility
   contract.
6. Add full TokaMaker equilibrium parity fixtures beyond the scalar
