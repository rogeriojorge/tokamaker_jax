...
stored-report summaries, editable TOML validation in the case browser,
...
extracted from the local OpenFUSIONToolkit checkout.
...
| Docs/examples | 100% | Installation, architecture, equations, derivations, design decisions, IO/artifact contracts, case manifest, upstream fixture inventory, fixed-boundary upstream evidence, porting map, API, examples, progress, validation manifest, upstream/literature comparisons, benchmark threshold docs, GUI/report notes, literature reproduction docs, and README quick-start are complete for this staged milestone. |
| Config/CLI | 100% | Default install now includes GUI dependencies, dependency metadata is unversioned, TOML runs support solver/grid/source/coil/output settings plus region geometry, `tokamaker-jax validate` checks configs without solving, `tokamaker-jax verify` runs all implemented gates, `tokamaker-jax cases` lists runnable examples plus planned upstream fixtures, `tokamaker-jax upstream-fixtures` summarizes exact source fixture files when available, and `tokamaker-jax fixed-boundary-evidence` records bounded source/gEQDSK evidence. |
| Test infra | 100% | Pytest, coverage, lint, docs, FEM analytic gates, weighted/axisymmetric assembly gates, true quadrature convergence gates, reduced and closed-form circular-loop coil gates, numeric OpenFUSIONToolkit `eval_green` parity when available, nonlinear profile/coupling gates, literature surrogate gates, validation CLI tests, GUI TOML-editor tests, benchmark threshold tests, CI workflow tests, packaging tests, upstream fixture snapshot tests, fixed-boundary evidence tests, and docs artifact tests are complete for this staged milestone. |
...
1. Convert the fixed-boundary source/stored-output evidence into a numeric
   equilibrium parity gate with explicit tolerances for flux, profiles, axis,
   current, q, and boundary diagnostics.
...
5. Promote the GUI TOML validator into one-click subprocess execution with
   artifact capture while preserving the current manifest-backed reproducibility
   contract.
